...
class BoilerDigitalTwin(DigitalTwinBase):
    ...
    def _simulate(self, state):
        predicted = min(100.0, max(0.0, self.predict_target(state)))
        steam = float(state["steam_flow_kg_hr"])
        fuel = float(state["fuel_flow_m3_hr"])
        return self._result(state, predicted_efficiency_percent=round(predicted, 2), steam_to_fuel_ratio=round(steam / max(fuel, 0.01), 3), thermal_loss_percent=round(100 - predicted, 2))
